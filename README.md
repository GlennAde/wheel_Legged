...
| 目录 | 大小 | 是什么 | 能否删除 / 如何重建 |
|---|---|---|---|
| `env_isaaclab/` | ~20G | **Python 虚拟环境**（uv 创建，Python 3.10）：Isaac Sim 4.5 pip 全家 + torch 2.7.0+cu128 + 全部依赖 | 可删。重建：`cd Isaac-RL-Two-wheel-Legged-Bot && ./setup_env.sh`（一键） |
| `IsaacLab/` | ~166M | **Isaac Lab 2.3.0 源码**（editable 安装：isaaclab / isaaclab_tasks / isaaclab_rl / isaaclab_assets / isaaclab_mimic，固定 commit `3c6e67bb`） | 可删。`setup_env.sh` 会自动从 git 固定 commit 重建（保留本地可加速安装） |
...
| `.opencode/` | 小 | opencode AI 工具的技能配置（含 isaaclab-env 技能），与训练无关 | 可留可删 |
...
Isaac-RL-Two-wheel-Legged-Bot (lab.flamingo 1.2.0)
   │
   ▼
训练入口：Isaac-RL-Two-wheel-Legged-Bot/run_train.sh
```
...
./setup_env.sh                          # 重建/修复环境（新人、换机器时用）
./diagnose_gpu.sh                       # GPU 自检（在自己终端跑）
./run_train.sh                          # 训练（默认 1024 envs, headless）
NUM_ENVS=512 ./run_train.sh             # 更省内存
HEADLESS=0 ./run_train.sh --max_iterations 30   # 打开窗口看可视化
./run_train.sh --video --max_iterations 200     # 录训练视频
./run_play.sh --load_run <时间戳>        # 评估 + 自动导出 ONNX
```
...
| 内容 | 建议 |
|---|---|
| `env_isaaclab/` | **不要备份/打包**（20G 且 .pth 写死本机路径）；迁移 = 重跑 `setup_env.sh` |
...
| 文档 | 位置 |
|---|---|
| 新人工作流指南（从零到部署的完整流程） | `Isaac-RL-Two-wheel-Legged-Bot/新人工作流指南.md` |
| 部署与 Isaac Sim 卡死排查 | `Isaac-RL-Two-wheel-Legged-Bot/部署与卡死排查.md` |
...
- ❌ 在 `wheel_legged` 根目录直接 `python xxx.py` —— 用仓库里的 `run_*.sh`
